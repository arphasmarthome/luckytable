...
export function Page({ children, scroll = true, padded = true, background = shell.canvas, style, contentStyle, gap = 24 }: { children: ReactNode; scroll?: boolean; padded?: boolean; background?: string; style?: StyleProp<ViewStyle>; contentStyle?: StyleProp<ViewStyle>; gap?: number }) {
  const { inset } = useBreakpoint();
  const padding = padded ? inset : 0;
