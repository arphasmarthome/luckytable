/* Home → 最近的回憶: the newest photo from the frame. Tap opens the photo frame on that photo;
 * swipe (or the arrows) steps through the album. */
    ...
    <HomeCard title={t("最近的回憶")} aside={current ? <Txt variant="meta" muted>{t("第 {n} 張照片", { n: (((index % count) + count) % count) + 1 })} / {count}</Txt> : undefined} style={{ flex: 1 }}>
      {current ? (
        <GestureDetector gesture={Gesture.Exclusive(pan, tap)}>
          <View accessibilityLabel={t("開啟相框")} style={{ flex: 1, minHeight: 140, borderRadius: radius.md, overflow: "hidden", backgroundColor: "#edf1ee" }}>
