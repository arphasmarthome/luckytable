/* One finished cooking session: a row of the dishes cooked, the family's own photos and one upload
 * tile at the end, then the start / pause / resume timeline. */
...
export function CookSummary({ record, compact }: { record: CookRecord; compact?: boolean }) {
  ...
  return (
    <View style={{ gap: compact ? 10 : 16 }}>
      <View style={{ flexDirection: "row", flexWrap: "wrap", alignItems: "flex-start", gap: compact ? 8 : 12 }}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={t.addPhotos} onPress={() => void upload()} style={({ pressed }) => [tile, { alignItems: "center", justifyContent: "center", gap: 4, borderWidth: 2, borderStyle: "dashed", borderColor: pressed ? make.primary : make.borderStrong, backgroundColor: pressed ? make.primarySoft : make.surface }]}>
