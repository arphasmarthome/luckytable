...
export function MemberBar({ view, date, horizontal }: { view: CalView; date: Date; horizontal?: boolean }) {
  ...
  return (
    <View accessibilityLabel={t("篩選家庭成員")} style={{ gap: 6 }}>
      <Txt variant="control" weight="600" color="#75857a" style={{ paddingHorizontal: 10, paddingBottom: 6 }}>
        {t("家庭成員")}
      </Txt>
      {people.map((person) => {
        ...
        return (
          <Pressable
            ...
            style={({ pressed }) => ({ flexDirection: "row", alignItems: "center", gap: 12, minHeight: 64, paddingVertical: 8, paddingHorizontal: 11, borderRadius: 7, borderWidth: 1, borderColor: active ? cal.activePersonBorder : "transparent", backgroundColor: active ? cal.activePerson : pressed ? "#eff4ef" : "transparent" })}>
            <View style={{ width: 39, height: 39, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: active ? "#fff" : tint(person.color, 0.1) }}>
...
export function Upcoming({ date, events }: { date: Date; events: CalEvent[] }) {
  ...
  return (
    <View style={{ paddingTop: 18, paddingHorizontal: 4 }}>
      <Txt variant="h3" color="#728477" style={{ marginBottom: 10 }}>
        {t("接下來")}
      </Txt>
      {upcoming.length ? (
        upcoming.map((event) => (
          <Pressable key={event.id} accessibilityRole="button" accessibilityLabel={t(event.title)} onPress={() => openEventDetail(event.id)} style={{ flexDirection: "row", alignItems: "flex-start", gap: 10, minHeight: 56, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#e7ece6" }}>
            <Txt variant="meta" weight="600" color="#74917d" style={{ width: 52, paddingTop: 3 }}>
              {event.time || t("全天")}
            </Txt>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Txt variant="body">{t(event.title)}</Txt>
              <Txt variant="meta" color="#86938b" style={{ marginTop: 4 }}>
...
export function Weather() {
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 14, borderTopWidth: 1, borderTopColor: cal.line, paddingTop: 16, paddingHorizontal: 4, marginTop: "auto" }}>
      <Icon name="cloud-sun" size={33} color={cal.weather} />
      <View style={{ flex: 1, flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <Txt variant="h1" color="#738678">
          24°
        </Txt>
